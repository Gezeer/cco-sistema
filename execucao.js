...
(function iniciarPaginaExecucao(){
  ...
  async function buscarEvolucaoServicoCCO(servico){
    const catalogoCompleto=await carregarCatalogoExecucaoCCO(),inicioHistorico=2025*12+11,catalogoBase=catalogoCompleto.filter(item=>Number(item.ano)*12+Number(item.mes)>=inicioHistorico).sort((a,b)=>Number(a.ano)-Number(b.ano)||Number(a.mes)-Number(b.mes)),catalogo=await Promise.all(catalogoBase.map(async periodo=>{const resolvida=await window.CCOPainelService.resolverImportacaoPeriodo(periodo.ano,periodo.mes,periodo.importacao_id);return resolvida?.importacao_id?{...periodo,importacao_id:resolvida.importacao_id,resolucaoImportacao:resolvida}:periodo;})),assinatura=catalogo.map(item=>`${periodoChave(item.ano,item.mes)}:${item.importacao_id}`).join("|"),chaveCache=`${VERSAO_CACHE_EVOLUCAO_EXECUCAO}|evolucao|${servico}|${assinatura}`,chaveCacheResolvida=`${VERSAO_RESOLUCAO_HISTORICO}|${chaveCache}`,armazenado=cacheEvolucaoExecucao.get(chaveCacheResolvida);
