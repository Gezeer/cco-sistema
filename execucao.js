...
(function iniciarPaginaExecucao(){
  ...
  async function buscarEvolucaoServicoCCO(servico){
    ...
    const resultadosFontes=await Promise.allSettled([banco.from("painel_executivo").select("importacao_id,ano,mes,servico,acumulado,previsto,valor_total").in("importacao_id",[...idsAtivos]).eq("servico",servico).order("ano",{ascending:true}).order("mes",{ascending:true}),carregarOperacoesHistorico(),Promise.allSettled(catalogo.map(periodo=>window.CCOPainelService.diasOperacaoPorPeriodo(periodo.importacao_id,periodo.ano,periodo.mes)))]),resposta=resultadosFontes[0].status==="fulfilled"?resultadosFontes[0].value:{data:[],error:resultadosFontes[0].reason},operacoes=resultadosFontes[1].status==="fulfilled"?resultadosFontes[1].value:[],resultadosDias=resultadosFontes[2].status==="fulfilled"?resultadosFontes[2].value:catalogo.map(()=>({status:"rejected",reason:resultadosFontes[2].reason}));
  ...
  async function renderizarEvolucaoHistoricaCCO(servicoSelecionado){
    ...
    if(!labels.length||previstos.length!==labels.length||acumulados.length!==labels.length){const error=new Error("Dataset histórico inválido para renderização.");console.error("[EXECUCAO GRAFICO ERRO]",error);return null;}
    ...
    if(!window.CCO_GRAFICOS_3D?.renderizarDireto){const error=new Error("Renderizador 3D indisponível.");console.error("[EXECUCAO GRAFICO ERRO]",error);return null;}let grafico;try{window.CCO_GRAFICOS_3D?.destruirGrafico?.(container);grafico=window.CCO_GRAFICOS_3D.renderizarDireto(container,{tipo:"barra3d",agrupado:true,preservarNulos:true,modoMobileCompacto:true,layoutRotulosExecucao:true,altura:460,categorias:labels,categoriasMobile:labelsMobile,grid:cfg=>cfg.mobile?{}:{top:100},yAxis:ehEquipe?{name:"Equipes",min:0}:undefined,series:[{nome:"Previsto",valores:previstos,cor:"#047857",corTopo:"#6ee7b7",corLateral:"#064e3b",formatarRotulo:(valor,indice)=>previstos[indice]==null?"":formatar(valor)},{nome:"Acumulado",valores:acumulados,cor:"#10b981",corTopo:"#a7f3d0",corLateral:"#065f46",formatarRotulo:(valor,indice)=>acumulados[indice]==null?"":formatar(valor)}],legend:{show:true,top:10,left:"center"},tooltip:{formatter:parametros=>{const itens=Array.isArray(parametros)?parametros:[parametros],indice=itens[0]?.dataIndex??0,pct=percentuais[indice];return`${labels[indice]}<br>Previsto: ${previstos[indice]===null?"Sem dados":`${formatar(previstos[indice])}${sufixoUnidade}`}<br>Acumulado: ${acumulados[indice]===null?"Sem dados":`${formatar(acumulados[indice])}${sufixoUnidade}`}<br>Percentual: ${pct===null?"Sem dados":`${pct.toLocaleString("pt-BR",{maximumFractionDigits:1})}%`}`;}}});if(!grafico)throw new Error("Renderizador não criou a instância do gráfico histórico.");}catch(error){console.error("[EXECUCAO GRAFICO ERRO]",error);return null;}
