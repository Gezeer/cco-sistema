...
(function iniciarPaginaExecucao(){
  ...
  async function buscarEvolucaoServicoCCO(servico){
    ...
    const carregarOperacoesHistorico=()=>medirHistorico("operacoes",async()=>{if(!precisaAgregadoOperacoes)return{data:[],error:null};return banco.from("operacoes").select("importacao_id,servico,acumulado:peso_t.sum()").in("importacao_id",[...idsAtivos]).eq("servico",servico).gte("data_operacao","2025-11-01");});
