...
(function iniciarPaginaExecucao(){
  ...
  async function buscarEvolucaoServicoCCO(servico){
    ...
    const colunasOperacoes=servico==="P4"?"id,importacao_id,servico,tipo_servico,data_operacao,peso_t":servico==="P1"?"id,importacao_id,servico,tipo_servico,data_operacao,viagens":"id,importacao_id,servico,tipo_servico,data_operacao,qtd_equipe,equipe";
    const carregarPainelHistorico=()=>medirHistorico("painel",()=>banco.from("painel_executivo").select("importacao_id,ano,mes,servico,acumulado").in("importacao_id",[...idsAtivos]).eq("servico",servico).order("ano",{ascending:true}).order("mes",{ascending:true}));
    ...
    const porPeriodo=new Map((resposta.data||[]).map(item=>[`${periodoChave(item.ano,item.mes)}|${String(item.importacao_id)}`,item])),mapaDias=new Map(diasPorPeriodo),linhas=catalogo.map(periodo=>{const importacaoId=String(periodo.importacao_id),chave=periodoChave(periodo.ano,periodo.mes),inicio=`${chave}-01`,fim=Number(periodo.mes)===12?`${Number(periodo.ano)+1}-01-01`:`${Number(periodo.ano)}-${pad(Number(periodo.mes)+1)}-01`,base=porPeriodo.get(`${chave}|${importacaoId}`)||{},registros=operacoes.filter(item=>String(item.importacao_id)===importacaoId&&String(item.data_operacao||"").slice(0,10)>=inicio&&String(item.data_operacao||"").slice(0,10)<fim);let acumulado=base.acumulado??null;if(servico==="P4")acumulado=resolverAcumuladoHistoricoP4CCO({periodo:{...periodo,importacao_id:importacaoId},operacoes:registros,acumuladoPainelExecutivo:base.acumulado}).acumulado;else if(window.CCOMetricas?.ehServicoEquipe?.(servico))acumulado=window.CCOMetricas.calcularEquipeMensalServico({servico,registros,ano:periodo.ano,mes:periodo.mes,importacaoId}).executado;else if(registros.length)acumulado=servico==="P1"?window.CCOMetricas.calcularAcumuladoP1Periodo({ano:periodo.ano,mes:periodo.mes,importacaoId,registros}):window.CCOMetricas.calcularAcumuladoServico(servico,registros);const totalDias=mapaDias.get(chave),previsto=totalDias?window.CCO_REGRAS.calcularPrevisto(servico,totalDias):null,linha={...base,importacao_id:importacaoId,ano:Number(periodo.ano),mes:Number(periodo.mes),servico,acumulado,previsto,total_dias_mes:totalDias,registros_historico:registros.length};if(window.CCO_DEBUG_PREVISTO_EXECUCAO===true||window.CCO_DEBUG_EXECUCAO_HISTORICO===true)console.log("[EXECUCAO HISTORICO PERIODO]",{periodo:chave,servico,importacaoId,totalDiasMes:totalDias,registros:registros.length,acumulado,previsto});return linha;});
