...
(function iniciarPaginaExecucao(){
  ...
  async function renderizarPeriodoExecucao(periodo){
    ...
    try{await window.carregarPeriodoCCO(normalizado);}catch(error){window.animarCardsExecucaoCCO?.({carregando:false,erro:error});throw error;}
  ...
  async function renderizarEvolucaoHistoricaCCO(servicoSelecionado){
    ...
    const grafico=window.CCO_GRAFICOS_3D?.renderizarDireto?.(container,{tipo:"barra3d",agrupado:true,preservarNulos:true,categorias:labels,categoriasMobile:labelsMobile,series:[{nome:"Previsto",valores:previstos,cor:"#047857",corTopo:"#6ee7b7",corLateral:"#064e3b",formatarRotulo:(valor,indice)=>previstos[indice]==null?"":formatar(valor)},{nome:"Acumulado",valores:acumulados,cor:"#10b981",corTopo:"#a7f3d0",corLateral:"#065f46",formatarRotulo:(valor,indice)=>acumulados[indice]==null?"":formatar(valor)}],legend:{show:true},tooltip:{formatter:parametros=>{const itens=Array.isArray(parametros)?parametros:[parametros],indice=itens[0]?.dataIndex??0,pct=percentuais[indice];return`${labels[indice]}<br>Previsto: ${previstos[indice]===null?"Sem dados":formatar(previstos[indice])}<br>Acumulado: ${acumulados[indice]===null?"Sem dados":formatar(acumulados[indice])}<br>Percentual: ${pct===null?"Sem dados":`${pct.toLocaleString("pt-BR",{maximumFractionDigits:1})}%`}`;}}});
