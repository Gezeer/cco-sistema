...
(function iniciarPaginaExecucao(){
  ...
  async function buscarEvolucaoServicoCCO(servico){
    ...
    let resolvidos=[];try{resolvidos=await medirHistorico("resolver",()=>performanceExecucao.medir("resolverImportacaoMs",()=>window.CCOPainelService.resolverImportacoesCatalogo(catalogoBase)));}catch(error){console.error("[EXECUCAO GRAFICO ERRO]",error);}
    ...
    if(armazenado){performanceExecucao.contar("cacheHits");performanceExecucao.contar("quantidadeRegistrosRecebidos",armazenado.linhas.length);if(window.CCO_DEBUG_EXECUCAO_PERFORMANCE===true)console.log("[EXEC HIST] volume",{periodos:catalogo.length,importacoes:catalogo.map(item=>item.importacao_id),operacoesCarregadas:0,paginasOperacoes:0,paginasPainel:0,cacheHits:1,cacheMisses:0});return{catalogo,linhas:armazenado.linhas,metricas};}
    ...
    performanceExecucao.contar("quantidadeRequests",metricas.requests);performanceExecucao.contar("quantidadeRegistrosRecebidos",(resposta.data||[]).length+agregadosOperacoes.length+resultadosDias.filter(item=>item?.status==="fulfilled"&&item.value).length);cacheEvolucaoExecucao.set(chaveCacheResolvida,{linhas});return{catalogo,linhas,metricas};
  }
  async function executarEvolucaoHistoricaCCO(servicoSelecionado){
    ...
    if(window.CCO_DEBUG_EXECUCAO_PERFORMANCE===true)console.timeEnd("[EXEC HIST] echarts");const duracaoRender=relogioExecucao()-inicioGrafico;performanceExecucao.contar("graficosMs",duracaoRender);performanceExecucao.contar("renderMs",duracaoRender);performanceExecucao.relatar();posicionarSecoesDetalheExecucao();atualizarTendenciaEvolucaoCCO(comparativo);return grafico;
  }
