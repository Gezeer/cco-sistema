...
(function iniciarPaginaExecucao(){
  ...
  const VERSAO_CACHE_EVOLUCAO_EXECUCAO="p4-operacoes-v2",VERSAO_RESOLUCAO_HISTORICO="importacao-periodo-compartilhada-v3",cacheEvolucaoExecucao=new Map();
  ...
  async function buscarEvolucaoServicoCCO(servico){
    const catalogoCompleto=await medirHistorico("catalogo",()=>carregarCatalogoExecucaoCCO()),inicioHistorico=2025*12+11,catalogoBase=catalogoCompleto.filter(item=>Number(item.ano)*12+Number(item.mes)>=inicioHistorico).sort((a,b)=>Number(a.ano)-Number(b.ano)||Number(a.mes)-Number(b.mes));
    ...
    const idsAtivos=new Set(catalogo.map(item=>String(item.importacao_id))),banco=window.supabaseClient,precisaOperacoes=servico==="P1"||servico==="P4"||window.CCOMetricas?.ehServicoEquipe?.(servico)===true;
    const colunasOperacoes=servico==="P4"||servico==="P1"?"id,importacao_id,servico,tipo_servico,data_operacao,peso_t":"id,importacao_id,servico,tipo_servico,data_operacao,qtd_equipe,equipe";
    ...
    const carregarPainelHistorico=()=>medirHistorico("painel",()=>banco.from("painel_executivo").select("importacao_id,ano,mes,servico,acumulado").in("importacao_id",[...idsAtivos]).order("ano",{ascending:true}).order("mes",{ascending:true}));
    const carregarOperacoesHistorico=()=>medirHistorico("operacoes",()=>precisaOperacoes?window.CCOSupabase.paginar(()=>banco.from("operacoes").select(colunasOperacoes).in("importacao_id",[...idsAtivos]).eq("servico",servico).gte("data_operacao","2025-11-01").order("data_operacao",{ascending:true}).order("id",{ascending:true})):Promise.resolve([]));
    const carregarDiasHistorico=()=>medirHistorico("dias",()=>Promise.allSettled(catalogo.map(periodo=>window.CCOPainelService.diasOperacaoPorPeriodo(periodo.importacao_id,periodo.ano,periodo.mes))));
    const resultadosFontes=await Promise.allSettled([carregarPainelHistorico(),carregarOperacoesHistorico(),carregarDiasHistorico()]),resposta=resultadosFontes[0].status==="fulfilled"?resultadosFontes[0].value:{data:[],error:resultadosFontes[0].reason},operacoes=resultadosFontes[1].status==="fulfilled"?resultadosFontes[1].value:[],resultadosDias=resultadosFontes[2].status==="fulfilled"?resultadosFontes[2].value:catalogo.map(()=>({status:"rejected",reason:resultadosFontes[2].reason}));
    for(const resultado of resultadosFontes)if(resultado.status==="rejected")console.error("[EXECUCAO GRAFICO ERRO]",resultado.reason);if(resposta.error)console.error("[EXECUCAO GRAFICO ERRO]",resposta.error);
    ...
    const normalizarServicoHistorico=valor=>window.CCOMetricas?.normalizarServico?.(valor)||String(valor||"").trim().toUpperCase(),servicoNormalizado=normalizarServicoHistorico(servico),porPeriodo=new Map();
    ...
    const mapaDias=new Map(diasPorPeriodo),linhas=catalogo.map(periodo=>{const importacaoId=String(periodo.importacao_id),chave=periodoChave(periodo.ano,periodo.mes),chavePainel=`${chave}|${importacaoId}|${servicoNormalizado}`,inicio=`${chave}-01`,fim=Number(periodo.mes)===12?`${Number(periodo.ano)+1}-01-01`:`${Number(periodo.ano)}-${pad(Number(periodo.mes)+1)}-01`,base=porPeriodo.get(chavePainel)||null,registros=operacoes.filter(item=>String(item.importacao_id)===importacaoId&&String(item.data_operacao||"").slice(0,10)>=inicio&&String(item.data_operacao||"").slice(0,10)<fim);let acumulado=base?.acumulado??null,fonteHistorico=base?"painel_executivo.acumulado":"indisponível";if(servico==="P4"){const resolvido=resolverAcumuladoHistoricoP4CCO({periodo:{...periodo,importacao_id:importacaoId},operacoes:registros,acumuladoPainelExecutivo:base?.acumulado});acumulado=resolvido.acumulado;fonteHistorico=resolvido.fonte;}else if(window.CCOMetricas?.ehServicoEquipe?.(servico)){acumulado=window.CCOMetricas.calcularEquipeMensalServico({servico,registros,ano:periodo.ano,mes:periodo.mes,importacaoId}).executado;fonteHistorico="operacoes.qtd_equipe/equipe + CCOMetricas.calcularEquipeMensalServico";}else if(servico==="P1"&&registros.length){acumulado=window.CCOMetricas.calcularAcumuladoP1Periodo({ano:periodo.ano,mes:periodo.mes,importacaoId,registros});fonteHistorico="operacoes.peso_t + CCOMetricas.calcularAcumuladoP1Periodo";}if(acumulado===null)console.error("[EXECUCAO HISTORICO ACUMULADO AUSENTE]",{periodo:chave,servico:servicoNormalizado,importacaoId,chaveLookup:chavePainel});const totalDias=mapaDias.get(chave),previsto=totalDias?window.CCO_REGRAS.calcularPrevisto(servico,totalDias):null,linha={...(base||{}),importacao_id:importacaoId,ano:Number(periodo.ano),mes:Number(periodo.mes),servico:servicoNormalizado,acumulado,previsto,total_dias_mes:totalDias,registros_historico:registros.length,fonte_historico:fonteHistorico},periodoAtual=periodoChave(window.filtroExecucaoAnoAtual,window.filtroExecucaoMesAtual);if(chave===periodoAtual){const card=(window.painelExecutivoOriginal||[]).find(item=>normalizarServicoHistorico(item.servico)===servicoNormalizado);console.log("[EXECUCAO CARD VS HISTORICO]",{periodo:chave,servico:servicoNormalizado,importacaoId,cardAcumulado:card?.acumulado_mes??null,historicoAcumulado:acumulado,fonteCard:card?.fonte_metricas||"indisponível",fonteHistorico});}if(window.CCO_DEBUG_PREVISTO_EXECUCAO===true||window.CCO_DEBUG_EXECUCAO_HISTORICO===true)console.log("[EXECUCAO HISTORICO PERIODO]",{periodo:chave,servico:servicoNormalizado,importacaoId,totalDiasMes:totalDias,registros:registros.length,acumulado,previsto,fonteHistorico});return linha;});
    if(window.CCO_DEBUG_EXECUCAO_PERFORMANCE===true){console.timeEnd("[EXEC HIST] montarDataset");console.log("[EXEC HIST] volume",{periodos:catalogo.length,importacoes:catalogo.map(item=>item.importacao_id),operacoesCarregadas:operacoes.length,paginasOperacoes:precisaOperacoes?Math.floor(operacoes.length/1000)+1:0,paginasPainel:1,cacheHits:0,cacheMisses:1,fonteAcumulado:precisaOperacoes?"operacoes filtradas pelo serviço + fallback painel_executivo":"painel_executivo agregado"});}
    cacheEvolucaoExecucao.set(chaveCacheResolvida,{linhas});return{catalogo,linhas};
  }
