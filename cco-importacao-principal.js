...
(function iniciarImportadorPrincipalCCO() {
  ...
  function extrairValorOperacionalP9(linha){
    ...
    return valor>0&&Number.isInteger(valor)?{valor,campo:possuiLiteral?"Qdt_Equipe":"qdt_equipe"}:{valor:null,campo:possuiLiteral?"Qdt_Equipe":possuiNormalizado?"qdt_equipe":null};
  }
  ...
  function obterQdtEquipeP9(linhaRaw){
    ...
    return{valor:valor>0&&Number.isInteger(valor)?valor:null,fonte,valorOriginal,cabecalhoLiteral};
  }
  ...
  function valoresOperacaoP9(item){return{qtd_equipe:normalizarNumero(item?.qtd_equipe),equipe:normalizarNumero(item?.equipe),executado:normalizarNumero(item?.executado)};}
  function operacaoP9Correta(existente,operacao){const atual=valoresOperacaoP9(existente),correto=normalizarNumero(operacao?.qtd_equipe);return correto>0&&Number.isInteger(correto)&&Object.values(atual).every(valor=>valor===correto);}
  function resumirOperacoesP9(operacoes){const registros=operacoes||[],valores=registros.flatMap(item=>Object.values(valoresOperacaoP9(item))).filter(valor=>valor!==null),valoresFracionarios=valores.filter(valor=>!Number.isInteger(valor)),valoresInteiros=valores.filter(Number.isInteger);return{registros:registros.length,valoresFracionarios:valoresFracionarios.length,valoresInteiros:valoresInteiros.length,menorValor:valores.length?Math.min(...valores):null,maiorValor:valores.length?Math.max(...valores):null,primeiras30:registros.slice(0,30).map(item=>({id:item.id,chave_operacao:item.chave_operacao,data_operacao:item.data_operacao,ra:item.ra,turno:item.turno,qtd_equipe:item.qtd_equipe,equipe:item.equipe,executado:item.executado}))};}
  function componentesRelacionamentoP9(item,importacaoId){return{importacao_id:texto(item?.importacao_id||importacaoId),servico:"P9",data_operacao:normalizarData(item?.data_operacao)||texto(item?.data_operacao),ra:normalizarCabecalhoCCO(item?.ra),turno:normalizarCabecalhoCCO(item?.turno),numero_linha:texto(item?.numero_linha)};}
  function chaveRelacionamentoP9(item,importacaoId){return JSON.stringify(componentesRelacionamentoP9(item,importacaoId));}
  function divergenciasRelacionamentoP9(raw,existente,importacaoId){const a=componentesRelacionamentoP9(raw,importacaoId),b=componentesRelacionamentoP9(existente,importacaoId);return Object.keys(a).filter(campo=>a[campo]!==b[campo]).map(campo=>({campo,raw:a[campo],existente:b[campo]}));}
  ...
  function prepararReprocessamentoP9({raw,ano,mes,importacaoId,chavesExistentes=[]}){
    ...
    const existentes=(chavesExistentes||[]).filter(item=>item&&typeof item==="object"),porChave=new Map();
    existentes.forEach(item=>{const chaveCanonica=chaveRelacionamentoP9(item,importacaoId),lista=porChave.get(chaveCanonica)||[];lista.push(item);porChave.set(chaveCanonica,lista);});
    const chavesRaw=new Set(geradas.map(item=>chaveRelacionamentoP9(item,importacaoId))),chavesExistentesCanonicas=new Set(existentes.map(item=>chaveRelacionamentoP9(item,importacaoId))),intersecoes=[...chavesRaw].filter(item=>chavesExistentesCanonicas.has(item));console.log("[P9 RELACIONAMENTO RESUMO]",{quantidadeChavesRaw:chavesRaw.size,quantidadeChavesExistentes:chavesExistentesCanonicas.size,quantidadeIntersecoes:intersecoes.length,quantidadeSemCorrespondencia:[...chavesRaw].filter(item=>!chavesExistentesCanonicas.has(item)).length});
    const vistas=new Set(),classificadas=geradas.map(operacao=>{const chaveCanonica=chaveRelacionamentoP9(operacao,importacaoId);let existente=null,candidatos=[],classificacao,motivo,divergencias=[];if(vistas.has(chaveCanonica)){classificacao="DUPLICADA_NO_PROPRIO_LOTE";motivo="A mesma chave canônica foi gerada mais de uma vez no RAW.";}else{vistas.add(chaveCanonica);candidatos=porChave.get(chaveCanonica)||[];if(candidatos.length>1){classificacao="AMBIGUIDADE_DE_VINCULO";motivo="Mais de uma operação existente possui a mesma chave canônica.";}else if(!candidatos.length){classificacao="INSERIR_OPERACAO_AUSENTE";motivo="Nenhuma operação existente possui os mesmos componentes de relacionamento do RAW.";const comparadas=existentes.map(item=>({item,divergencias:divergenciasRelacionamentoP9(operacao,item,importacaoId)})).sort((a,b)=>a.divergencias.length-b.divergencias.length);divergencias=comparadas[0]?.divergencias||[];}else{existente=candidatos[0];divergencias=divergenciasRelacionamentoP9(operacao,existente,importacaoId);if(operacaoP9Correta(existente,operacao)){classificacao="OPERACAO_CORRETA";motivo="qtd_equipe, equipe e executado são exatamente iguais ao Qdt_Equipe do RAW.";}else{classificacao="ATUALIZAR";motivo="Um ou mais campos diferem do Qdt_Equipe do RAW.";}}}const atual=valoresOperacaoP9(existente);console.log("[P9 RELACIONAMENTO RAW → OPERAÇÃO]",{chave_operacao_calculada:operacao.chave_operacao,chave_operacao_existente_encontrada:existente?.chave_operacao??null,id_encontrado:existente?.id??null,motivo_sem_correspondencia:existente?null:motivo,divergencias});console.log("[P9 CLASSIFICAÇÃO]",{id:existente?.id??null,qtd_equipe_atual:atual.qtd_equipe,equipe_atual:atual.equipe,executado_atual:atual.executado,valor_raw:operacao.qtd_equipe,classificacao,motivo});return{operacao,existente,candidatos,classificacao,motivo,divergencias};});
    return{identificadas,rejeitadasIdentificacao,descartadas,geradas,classificadas,jaExistentes:classificadas.filter(item=>item.classificacao==="OPERACAO_CORRETA"),corrigirExistentes:classificadas.filter(item=>item.classificacao==="ATUALIZAR"),ambiguidades:classificadas.filter(item=>item.classificacao==="AMBIGUIDADE_DE_VINCULO"),duplicadasNoLote:classificadas.filter(item=>item.classificacao==="DUPLICADA_NO_PROPRIO_LOTE"),prontas:classificadas.filter(item=>item.classificacao==="INSERIR_OPERACAO_AUSENTE").map(item=>item.operacao)};
  }
  ...
  async function reprocessarP9Periodo({ano,mes,importacaoId}={}) {
    ...
    const existentes=await buscarTudoPaginado(()=>cliente.from("operacoes").select("id,importacao_id,chave_operacao,data_operacao,qtd_equipe,equipe,executado,ra,turno,numero_linha").eq("importacao_id",id).eq("servico","P9").order("id",{ascending:true})),preparacao=prepararReprocessamentoP9({raw,ano:anoNumero,mes:mesNumero,importacaoId:id,chavesExistentes:existentes}),erros=[],resumoAntes=resumirOperacoesP9(existentes);
    console.log("[P9 ANTES DA CORREÇÃO]",{importacaoId:id,...resumoAntes});
    ...
    preparacao.classificadas.filter(item=>item.classificacao!=="INSERIR_OPERACAO_AUSENTE").forEach(item=>console.log("[P9 DEDUPLICAÇÃO]",{classificacao:item.classificacao,chaveOperacao:item.operacao.chave_operacao,numeroLinha:item.operacao.numero_linha}));
    ...
    const exemplosAntesDepois=preparacao.corrigirExistentes.slice(0,30).map(item=>({id:item.existente.id,chave_operacao:item.existente.chave_operacao,valorAnterior:valoresOperacaoP9(item.existente),valorCorreto:item.operacao.qtd_equipe,fonte:"Qdt_Equipe"})),previa={ano:anoNumero,mes:mesNumero,importacaoId:id,rawTotal:raw.length,rawIdentificadoComoP9:preparacao.identificadas.length,linhasComDataValida,linhasComQdtEquipePositivo,operacoesGeradas:preparacao.geradas.length,corretas:preparacao.jaExistentes.length,existentesParaCorrigir:preparacao.corrigirExistentes.length,ausentesParaInserir:preparacao.prontas.length,ambiguidades:preparacao.ambiguidades.length,semQdtEquipe:preparacao.descartadas.filter(item=>item.motivo==="P9_QDT_EQUIPE_AUSENTE").length,duplicadasNoLote:preparacao.duplicadasNoLote.length,descartadasPorMotivo,exemplosAntesDepois};
    console.log("[P9 CORREÇÃO PRÉVIA]",previa);console.table(exemplosAntesDepois);console.table(preparacao.prontas.slice(0,30));
    if(preparacao.ambiguidades.length){console.error("[P9 AMBIGUIDADE DE VÍNCULO]",preparacao.ambiguidades);throw new Error(`P9 ${periodo}: ${preparacao.ambiguidades.length} ambiguidade(s) de vínculo. Nenhuma gravação foi executada.`);}
    if(preparacao.prontas.length===0&&preparacao.corrigirExistentes.length===0){const resultadoSemAlteracao={periodo,importacaoId:id,quantidadeAtualizada:0,quantidadeInserida:0,quantidadeJaCorreta:preparacao.jaExistentes.length,erros:[],cancelado:false};console.log("[P9 REPROCESSAMENTO RESULTADO]",resultadoSemAlteracao);return resultadoSemAlteracao;}
    ...
    for(const lote of erros.length?[]:lotesAdaptativos(preparacao.prontas,50)){
      ...
      const inseridasNoLote=(resposta.data||[]).length;
      console.log("[P9 INSERT DEPOIS]",{importacaoId:id,periodo,quantidadeEnviada:lote.length,quantidadeRealmenteInserida:inseridasNoLote,erroSupabase:resposta.error?{status:resposta.status,code:resposta.error.code,message:resposta.error.message,details:resposta.error.details,hint:resposta.error.hint}:null,primeiraLinhaEnviada:lote[0]||null});
      if(resposta.error){const erro={etapa:"insert_p9",status:resposta.status,code:resposta.error.code,message:resposta.error.message,details:resposta.error.details,hint:resposta.error.hint};erros.push(erro);console.error("[P9 INSERT ERRO]",erro);break;}
      inseridas+=inseridasNoLote;
    }
    const depois=await buscarTudoPaginado(()=>cliente.from("operacoes").select("id,importacao_id,chave_operacao,data_operacao,qtd_equipe,equipe,executado,ra,turno,numero_linha").eq("importacao_id",id).eq("servico","P9").gte("data_operacao",inicio).lt("data_operacao",fim).order("id",{ascending:true})),diasDistintosDepois=new Set(depois.map(item=>item.data_operacao).filter(Boolean)).size,somaQtdEquipeDepois=depois.reduce((s,item)=>s+(normalizarNumero(item.qtd_equipe)||0),0),maximoEquipeNoMes=depois.reduce((max,item)=>Math.max(max,normalizarNumero(item.qtd_equipe)||0),0),valoresDepois=depois.flatMap(item=>Object.values(valoresOperacaoP9(item))),registrosInvalidosDepois=depois.filter(item=>{const valores=Object.values(valoresOperacaoP9(item));return valores.some(valor=>valor===null||!(valor>0)||!Number.isInteger(valor))||new Set(valores).size!==1;}),valoresFracionariosRestantes=registrosInvalidosDepois.length,distribuicaoValores=valoresDepois.reduce((mapa,valor)=>(mapa[String(valor)]=(mapa[String(valor)]||0)+1,mapa),{}),resultado={periodo,importacaoId:id,quantidadeAtualizada:corrigidas,quantidadeInserida:inseridas,quantidadeJaCorreta:preparacao.jaExistentes.length,ambiguidades:preparacao.ambiguidades.length,semQdtEquipe:previa.semQdtEquipe,erros,antes:resumoAntes,operacoesP9Depois:depois.length,diasDistintosDepois,somaQtdEquipeDepois,maximoEquipeNoMes,valoresFracionariosRestantes,distribuicaoValores};
    console.log("[P9 DEPOIS DA CORREÇÃO]",{registros:depois.length,atualizadas:corrigidas,inseridas,erros,valoresFracionariosRestantes,diasDistintos:diasDistintosDepois,maiorEquipe:maximoEquipeNoMes,distribuicaoValores});
    if(!erros.length&&valoresFracionariosRestantes>0)throw new Error(`P9 ${periodo}: validação falhou; restaram ${valoresFracionariosRestantes} valor(es) inválido(s) ou fracionário(s).`);
