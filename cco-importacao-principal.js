...
(function iniciarImportadorPrincipalCCO() {
  ...
  function prepararReprocessamentoP9({raw,ano,mes,importacaoId,chavesExistentes=[]}){
    ...
    existentes.forEach(item=>{if(!item.chave_operacao)return;const lista=porChave.get(item.chave_operacao)||[];lista.push(item);porChave.set(item.chave_operacao,lista);});
    const vistas=new Set(),classificadas=geradas.map(operacao=>{let existente=null,candidatos=[],classificacao,motivo;if(vistas.has(operacao.chave_operacao)){classificacao="DUPLICADA_NO_PROPRIO_LOTE";motivo="A mesma chave_operacao foi gerada mais de uma vez no RAW.";}else{vistas.add(operacao.chave_operacao);candidatos=porChave.get(operacao.chave_operacao)||[];if(candidatos.length>1){classificacao="AMBIGUIDADE_DE_VINCULO";motivo="Mais de uma operação existente possui a mesma chave_operacao.";}else if(!candidatos.length){classificacao="INSERIR_OPERACAO_AUSENTE";motivo="Nenhuma operação existente possui a chave_operacao gerada pelo RAW.";}else{existente=candidatos[0];if(operacaoP9Correta(existente,operacao)){classificacao="OPERACAO_CORRETA";motivo="qtd_equipe, equipe e executado são exatamente iguais ao Qdt_Equipe do RAW.";}else{classificacao="ATUALIZAR";motivo="Um ou mais campos diferem do Qdt_Equipe do RAW.";}}}const atual=valoresOperacaoP9(existente);console.log("[P9 CLASSIFICAÇÃO]",{id:existente?.id??null,qtd_equipe_atual:atual.qtd_equipe,equipe_atual:atual.equipe,executado_atual:atual.executado,valor_raw:operacao.qtd_equipe,classificacao,motivo});return{operacao,existente,candidatos,classificacao,motivo};});
