...
(function iniciarImportadorPrincipalCCO() {
  ...
  function chaveVinculoOperacaoP9(item){return[texto(item?.importacao_id),"P9",texto(item?.data_operacao),normalizarCabecalhoCCO(item?.ra),normalizarCabecalhoCCO(item?.turno),texto(item?.numero_linha)].join("|");}
  ...
  function prepararReprocessamentoP9({raw,ano,mes,importacaoId,chavesExistentes=[]}){
    ...
    const existentes=(chavesExistentes||[]).filter(item=>item&&typeof item==="object"),chavesLegadas=new Set((chavesExistentes||[]).filter(item=>typeof item==="string")),porChave=new Map(),porVinculo=new Map();
    existentes.forEach(item=>{if(item.chave_operacao){const lista=porChave.get(item.chave_operacao)||[];lista.push(item);porChave.set(item.chave_operacao,lista);}const vinculo=chaveVinculoOperacaoP9(item),lista=porVinculo.get(vinculo)||[];lista.push(item);porVinculo.set(vinculo,lista);});
    const vistas=new Set(),classificadas=geradas.map(operacao=>{if(vistas.has(operacao.chave_operacao))return{operacao,existente:null,candidatos:[],classificacao:"DUPLICADA_NO_PROPRIO_LOTE"};vistas.add(operacao.chave_operacao);if(chavesLegadas.has(operacao.chave_operacao))return{operacao,existente:null,candidatos:[],classificacao:"OPERACAO_CORRETA"};const exatos=porChave.get(operacao.chave_operacao)||[],candidatos=exatos.length?exatos:(porVinculo.get(chaveVinculoOperacaoP9(operacao))||[]);if(candidatos.length>1)return{operacao,existente:null,candidatos,classificacao:"AMBIGUIDADE_DE_VINCULO"};if(!candidatos.length)return{operacao,existente:null,candidatos,classificacao:"INSERIR_OPERACAO_AUSENTE"};const existente=candidatos[0];return{operacao,existente,candidatos,classificacao:operacaoP9Correta(existente,operacao)?"OPERACAO_CORRETA":"CORRIGIR_OPERACAO_EXISTENTE"};});
    return{identificadas,rejeitadasIdentificacao,descartadas,geradas,classificadas,jaExistentes:classificadas.filter(item=>item.classificacao==="OPERACAO_CORRETA"),corrigirExistentes:classificadas.filter(item=>item.classificacao==="CORRIGIR_OPERACAO_EXISTENTE"),ambiguidades:classificadas.filter(item=>item.classificacao==="AMBIGUIDADE_DE_VINCULO"),duplicadasNoLote:classificadas.filter(item=>item.classificacao==="DUPLICADA_NO_PROPRIO_LOTE"),prontas:classificadas.filter(item=>item.classificacao==="INSERIR_OPERACAO_AUSENTE").map(item=>item.operacao)};
  }
  ...
  async function reprocessarP9Periodo({ano,mes,importacaoId}={}) {
    ...
    if(preparacao.prontas.length===0&&preparacao.corrigirExistentes.length===0)throw new Error(`P9 ${periodo}: nenhuma operação pronta para inserir ou corrigir. Consulte [P9 REPROCESSAMENTO PRÉVIA], descartes e deduplicação.`);
    ...
    for(const item of preparacao.corrigirExistentes){
      const valorQdtEquipe=item.operacao.qtd_equipe,payload={servico:"P9",tipo_servico:"P9",qtd_equipe:valorQdtEquipe,equipe:valorQdtEquipe,executado:valorQdtEquipe};
      console.log("[P9 CORREÇÃO GRAVAÇÃO]",{importacaoId:id,idOperacao:item.existente.id,numeroLinha:item.operacao.numero_linha,valorAnterior:{qtd_equipe:item.existente.qtd_equipe,equipe:item.existente.equipe,executado:item.existente.executado},Qdt_Equipe:valorQdtEquipe,payload});
