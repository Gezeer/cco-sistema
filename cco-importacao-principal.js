...
(function iniciarImportadorPrincipalCCO() {
  ...
  function prepararReprocessamentoP9({raw,ano,mes,importacaoId,chavesExistentes=[]}){
    ...
    const registrosExistentes=new Map((chavesExistentes||[]).filter(Boolean).map(item=>typeof item==="object"?[item.chave_operacao,item]:[item,null]).filter(([chaveExistente])=>chaveExistente)),vistas=new Set(),classificadas=geradas.map(operacao=>{let classificacao="ACEITA_PARA_INSERT",existente=null;if(operacao.chave_operacao&&registrosExistentes.has(operacao.chave_operacao)){existente=registrosExistentes.get(operacao.chave_operacao);const valoresCorretos=!existente||["qtd_equipe","equipe","executado"].every(campo=>normalizarNumero(existente[campo])===operacao[campo]);classificacao=valoresCorretos?"CHAVE_OPERACAO_JA_EXISTENTE":"CORRIGIR_OPERACAO_EXISTENTE";}else if(operacao.chave_operacao&&vistas.has(operacao.chave_operacao))classificacao="DUPLICADA_NO_PROPRIO_LOTE";if(operacao.chave_operacao)vistas.add(operacao.chave_operacao);return{operacao,existente,classificacao};});
    return{identificadas,rejeitadasIdentificacao,descartadas,geradas,classificadas,jaExistentes:classificadas.filter(item=>item.classificacao==="CHAVE_OPERACAO_JA_EXISTENTE"),corrigirExistentes:classificadas.filter(item=>item.classificacao==="CORRIGIR_OPERACAO_EXISTENTE"),duplicadasNoLote:classificadas.filter(item=>item.classificacao==="DUPLICADA_NO_PROPRIO_LOTE"),prontas:classificadas.filter(item=>item.classificacao==="ACEITA_PARA_INSERT").map(item=>item.operacao)};
  }
  ...
  async function reprocessarP9Periodo({ano,mes,importacaoId}={}) {
    ...
    const previa={ano:anoNumero,mes:mesNumero,importacaoId:id,rawTotal:raw.length,rawIdentificadoComoP9:preparacao.identificadas.length,linhasComDataValida,linhasComQdtEquipePositivo,operacoesGeradas:preparacao.geradas.length,jaExistentes:preparacao.jaExistentes.length,existentesParaCorrigir:preparacao.corrigirExistentes.length,duplicadasNoLote:preparacao.duplicadasNoLote.length,prontasParaInserir:preparacao.prontas.length,descartadasPorMotivo};
    ...
    for(const item of preparacao.corrigirExistentes){
      ...
      if(resposta.error){const erro={etapa:"update_p9_existente",status:resposta.status,code:resposta.error.code,message:resposta.error.message,details:resposta.error.details,hint:resposta.error.hint};erros.push(erro);console.error("[P9 INSERT ERRO]",erro);break;}
