...
(function iniciarImportadorPrincipalCCO() {
  ...
  function prepararReprocessamentoP9({raw,ano,mes,importacaoId,chavesExistentes=[]}){
    ...
    const existentes=(chavesExistentes||[]).filter(item=>item&&typeof item==="object"),chavesLegadas=new Set((chavesExistentes||[]).filter(item=>typeof item==="string")),porChave=new Map();
    ...
    const vistas=new Set(),classificadas=geradas.map(operacao=>{if(vistas.has(operacao.chave_operacao))return{operacao,existente:null,candidatos:[],classificacao:"DUPLICADA_NO_PROPRIO_LOTE"};vistas.add(operacao.chave_operacao);if(chavesLegadas.has(operacao.chave_operacao))return{operacao,existente:null,candidatos:[],classificacao:"OPERACAO_CORRETA"};const candidatos=porChave.get(operacao.chave_operacao)||[];if(candidatos.length>1)return{operacao,existente:null,candidatos,classificacao:"AMBIGUIDADE_DE_VINCULO"};if(!candidatos.length)return{operacao,existente:null,candidatos,classificacao:"INSERIR_OPERACAO_AUSENTE"};const existente=candidatos[0];return{operacao,existente,candidatos,classificacao:operacaoP9Correta(existente,operacao)?"OPERACAO_CORRETA":"CORRIGIR_OPERACAO_EXISTENTE"};});
