...
assert.match(responsivo,/altura=Math\.max\(680,dados\.length\*46\+120\)/);
...
assert.match(responsivo,/barWidth:18,barCategoryGap:"55%"/);
assert.match(responsivo,/grid:\{left:72,right:24,top:24,bottom:58,containLabel:true\}/);
...
for(const campo of["Serviço:","Valor contratado:","Acumulado:","Previsto:","Percentual:"])assert.ok(responsivo.includes(campo),`tooltip deve conter ${campo}`);
assert.match(responsivo,/confine:true,appendToBody:false/);

assert.match(css,/@media\(max-width:767px\)[\s\S]*#graficoFinanceiro[\s\S]*height:var\(--cco-chart-mobile-height,718px\)!important[\s\S]*min-height:680px!important[\s\S]*max-height:780px!important[\s\S]*overflow:hidden!important/);
...
assert.match(html,/css\/painel-geral\.css\?v=20260805-painel-valor-servico-mobile-v1/);
assert.match(html,/js\/cco-graficos-3d\.js\?v=20260805-painel-valor-servico-mobile-v1/);
assert.match(html,/painel-geral\.js", "20260805-painel-valor-servico-mobile-v1"/);
