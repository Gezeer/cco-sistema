...
const secao={hidden:false};contexto.document={getElementById:id=>id==="comparativoTitulo"?{closest:()=>secao}:null};assert.equal(C.definirVisibilidadeComparativo(""),true);assert.equal(secao.hidden,false);assert.equal(C.definirVisibilidadeComparativo("2025"),false);assert.equal(secao.hidden,true);assert.equal(C.definirVisibilidadeComparativo("2026"),false);assert.equal(secao.hidden,true);assert.equal(C.definirVisibilidadeComparativo(""),true);assert.equal(secao.hidden,false);
...
assert.match(pagina,/filtrarLocal\(\{ignorarAno:true\}\)/);assert.equal((pagina.match(/S\(\)\.consultar\(/g)||[]).length,1);assert.match(service,/global\.CCOCache\?\.invalidar\?\.\("interrupcao"\)/);assert.match(html,/interrupcao-trecho-comparativo\.js\?v=20260816-interrupcao-comparativo-visibilidade-v2/);assert.match(css,/\.comparativo-graficos/);assert.match(css,/@media\(max-width:768px\)\{\.comparativo-cards,\.comparativo-graficos\{grid-template-columns:1fr\}/);
assert.match(css,/\.comparativo-anual\[hidden\]\{display:none!important\}/);assert.match(fs.readFileSync("interrupcao-trecho-comparativo.js","utf8"),/Object\.keys\(charts\)\.forEach\(destruir\)/);
