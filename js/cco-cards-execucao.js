(function configurarCardsExecucaoCCO(global){
  ...
  function animarNumeroCardCCO(elemento,valorFinal,opcoes={}){
    ...
    const inicio=Number.isFinite(Number(opcoes.valorInicial))?Number(opcoes.valorInicial):numeroPtBR(elemento.textContent)??0,limite=global.CCOEhMobile?.()?350:800,duracao=Math.min(limite,Math.max(0,Number(opcoes.duracao??650))),casas=Math.max(0,Number(opcoes.casas??0)),prefixo=opcoes.prefixo??"",sufixo=opcoes.sufixo??"";
