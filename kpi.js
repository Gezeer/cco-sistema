...
(function iniciarPaginaKpi3D(){
  ...
  function mostrar(id,visivel){const card=$(id)?.closest?.(".section, .chart-card");if(card)card.style.display=visivel?"":"none";}
  function render(id,tipo,categorias,series,opcoes={}){const container=$(id),possui=series.some(item=>tem(item.valores));mostrar(id,possui);G()?.destruirGrafico?.(container);if(!container||!possui)return null;return G()?.renderizarDireto?.(container,{tipo,categorias,series,...opcoes});}
  ...
  window.renderGraficosKpiServicoCompleto=function renderGraficosKpiServicoCompleto3D(dados,filtro={},painel={}){
    ...
    render("graficoKpiServicoDiario","cilindro",diario.map(([data])=>typeof window.formatarDataBRSimples==="function"?window.formatarDataBRSimples(data):data),[{nome:"Executado diário",valores:diario.map(([,valor])=>valor),cor:executadoCor}]);
    ...
    render("graficoKpiPrevistoExecutado","cilindro",["Previsto","Executado"],[{nome:"Previsto x Executado",valores:[previstoTotal,executadoTotal],cor:executadoCor}]);
    render("graficoKpiServicoMensal","cilindro",labels,[{nome:"Executado mensal",valores:mensal.map(item=>n(item.executado)),cor:executadoCor}]);
    ...
    render("graficoKpiServicoIndicadores","rosca",["Peso","Viagens","KM","Equipes"],[{nome:"Indicadores",valores:indicadores}],{radius:raioIndicadores,center:["50%","58%"],legend:{top:6,left:"center",orient:"horizontal",type:"scroll",itemWidth:16,itemHeight:10,itemGap:14,textStyle:{fontSize:fonteIndicadores,color:"#ecfdf5"}},label:{show:larguraTela>480,fontSize:fonteIndicadores,lineHeight:14,distanceToLabelLine:4,formatter:parametros=>`${parametros.name}\n${parametros.percent}%`},labelLine:{show:larguraTela>480,length:10,length2:8,smooth:.25},tooltip:{formatter:parametros=>`${parametros.name}<br>Valor: ${n(parametros.value).toLocaleString("pt-BR",{maximumFractionDigits:2})}<br>Participação: ${n(parametros.percent).toLocaleString("pt-BR",{maximumFractionDigits:2})}%`}});
    ...
    const graficoComparativo=render("graficoKpiComparativoMensal","cilindro",labelsComparativo,[{nome:"Previsto",valores:seriePrevisto,cor:previstoCor},{nome:"Executado",valores:serieExecutado,cor:executadoCor}],{tooltip:{formatter:parametros=>{const itens=Array.isArray(parametros)?parametros:[parametros],titulo=itens[0]?.axisValueLabel||itens[0]?.name||"";return[titulo,...itens.map(item=>`${item.marker||""}${item.seriesName}: ${n(item.value?.[1]??item.value).toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})}`)].join("<br>");}}});
    graficoComparativo?.setOption?.({yAxis:{max:maiorValor}},false);
    render("graficoKpiProdutividadeMensal","cilindro",labels,[{nome:"Ton/viagem",valores:mensal.map(i=>n(i.viagens)?n(i.peso_t)/n(i.viagens):0),cor:cores.peso||"#0f766e"},{nome:"KM/viagem",valores:mensal.map(i=>n(i.viagens)?n(i.km_total)/n(i.viagens):0),cor:cores.km||"#22c55e"}]);
