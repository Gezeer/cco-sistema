...
(function iniciarPaginaKpi3D(){
  ...
  window.renderGraficosKpiServicoCompleto=function renderGraficosKpiServicoCompleto3D(dados,filtro={},painel={}){
    ...
    if(equipeDiaria){const previstosDiarios=equipeDiaria.map(item=>item.previsto);render("graficoKpiServicoDiario",cfg=>cfg.mobile?"linha":"cilindro",categoriasDiarias,[{nome:"Previsto",valores:previstosDiarios,cor:previstoCor},{nome:"Executado",valores:valoresDiarios,cor:executadoCor}],{preservarNulos:true,yAxis:{name:"Equipes",min:0},tooltip:{formatter:parametros=>{const itens=Array.isArray(parametros)?parametros:[parametros],indice=itens[0]?.dataIndex??0,previsto=previstosDiarios[indice],executado=valoresDiarios[indice],diferenca=executado-previsto,percentual=executado/previsto*100,formatar=valor=>Number(valor).toLocaleString("pt-BR",{maximumFractionDigits:2});return`${categoriasDiarias[indice]}<br>Previsto: ${formatar(previsto)} equipes<br>Executado: ${formatar(executado)} equipes<br>Diferença: ${formatar(diferenca)}<br>Execução: ${percentual.toLocaleString("pt-BR",{maximumFractionDigits:2})}%`;}}});}else render("graficoKpiServicoDiario",cfg=>cfg.mobile?"linha":"cilindro",categoriasDiarias,[{nome:"Executado diário",valores:valoresDiarios,cor:executadoCor}]);
