...
assert.match(css,/\.kpi-producao-mensal-card,[\s\S]*\.kpi-indicadores-operacionais-card[\s\S]*min-height: 520px !important/);
assert.match(css,/\.kpi-producao-mensal-card[\s\S]*grid-template-rows: auto minmax\(350px, 1fr\)[\s\S]*row-gap: 16px/);
assert.match(css,/\.kpi-producao-mensal-grafico[\s\S]*height: 100% !important[\s\S]*padding-bottom: 20px/);
...
assert.match(html,/css\/kpi\.css\?v=20260805-kpi-cards-espacamento-v1/);
